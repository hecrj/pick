...
/// The most lines kept verbatim at the head and tail of the output;
/// beyond this, the middle is dropped.
const CAP: usize = 500;

/// A bounded, line-oriented capture of a tool's output.
///
/// The first `CAP` lines are kept, as are the most recent `CAP`;
...
impl Output {
    ...
    /// The first `n` lines, in reading order; `n` is capped at `CAP`.
    pub fn head(&self, n: usize) -> impl Iterator<Item = &str> {
        self.head.iter().take(n.min(CAP)).map(String::as_str)
    }

    /// The most recent `n` lines, in reading order; `n` is capped at `CAP`.
...
mod tests {
    ...
    fn a_long_output_keeps_only_its_head_and_tail() {
        ...
        assert_eq!(output.lines(), CAP * 3);
        assert_eq!(output.lines_ellided(), CAP);
        assert_eq!(output.head(1).collect::<Vec<_>>(), ["line 0"]);
        assert_eq!(
            output.head(CAP).collect::<Vec<_>>().last().copied(),
            Some(format!("line {}", CAP - 1).as_str())
        );
        assert_eq!(
            output.tail(CAP).collect::<Vec<_>>().first().copied(),
            Some(format!("line {}", CAP * 2).as_str())
        );
        assert_eq!(
            output.tail(1).collect::<Vec<_>>(),
            [format!("line {}", CAP * 3 - 1).as_str()]
        );
        assert_eq!(output.all().count(), CAP * 2);
        // `n` is capped at `CAP`.
        assert_eq!(
            output.head(CAP * 3).collect::<Vec<_>>(),
            output.head(CAP).collect::<Vec<_>>()
        );
        assert_eq!(
            output.tail(CAP * 3).collect::<Vec<_>>(),
            output.tail(CAP).collect::<Vec<_>>()
        );
        ...
        assert!(rendered.ends_with(&format!("line {}", CAP * 3 - 1)));
    }
